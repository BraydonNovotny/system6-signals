...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  <details class="changelog">
    <summary>Strategy changelog — 3 updates (2026-07-27)</summary>
    <div class="tab-bar">
...
function render(dateStr) {
  ...
  if (!taken.length) {
    ...
    const rows = taken.map(s => {
      ...
      if (s.firstTradeOfDay && s.sizeMult === 0.25) resultHtml += ' <span class="gap-badge" style="background:color-mix(in srgb, var(--loss) 20%, transparent); color:var(--loss);" title="Day\\'s first taken trade AND below median 20-day $ volume liquidity -- two independent weak-spot signals compounding. Locked rule: size to 0.25x total (0.5x first-trade x 0.5x thin-liquidity).">0.25x</span>';
      else if (s.firstTradeOfDay && s.sizeMult === 0.5) resultHtml += ' <span class="gap-badge" style="background:color-mix(in srgb, var(--text-faint) 25%, transparent); color:var(--text-mute);" title="Day\\'s first taken trade -- diagnostic showed a materially worse win% here across every core tier. Locked rule: size to 0.5x (EP/Parabolic/q0.5 confluence are exempt).">0.5x</span>';
      else if (s.thinFirstTrade && s.sizeMult === 0.5) resultHtml += ' <span class="gap-badge" style="background:color-mix(in srgb, var(--text-faint) 25%, transparent); color:var(--text-mute);" title="Day\\'s first taken trade AND below median liquidity (EP/Parabolic/q0.5 exempt from the base first-trade rule, but not this one). Locked rule: size to 0.5x.">0.5x (thin)</span>';
      const tf = s.tf || '30m';
